...
const express = require('express') // not sure why express wont light up? eslint error?
...
router.get('/api/actions/:id', async (req,res) =>{
    ...
    try{
     ...
     if (!action) {
      res.status(404).json({ message: "the action with this id does not exsist"})
     }else {
   ...
   try {
    ...
    if (!deletedAction) {
     res.status(404).json({ message: "the actions with that id does not exsist"})
    } else {
   ...
   router.use((err,req,res /*next*/) => {
    res.status(500).json({
     message:"Sorry, there has been a catastrophe",
     error:err.message
    })
   })

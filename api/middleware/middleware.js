...
const checkProjectsId = async (req,res,next) => {
 ...
 try {
  ...
  if(!project){
   res.status(400).json({message:`theres no project with this id: ${id}`})
  }else{
   req.project = project
   next()
  }
...
const checkActionsId = async (req,res,next) => {
 ...
 try{
  ...
  if(!action){
   res.status(400).json({message:`no project found with this id: ${id}`})
  }else{
   req.action = action
   next()
  }
...
const checkActionValidation = (req,res,next) =>{
    if (!req.body.name || !req.body.description) {
        res.status(400).json({ message: 'user requires both name & description' })
    } else {
        next()
    }
}
